...
-- #TABLES
CREATE TABLE microblog_comment ( id INT NOT NULL PRIMARY KEY DEFAULT nextval( 'microblog_comment_id' ), "date" TIMESTAMP NOT NULL, author INT NOT NULL, content VARCHAR( 150 ) NOT NULL, hash VARCHAR( 64 ) UNIQUE NOT NULL );
CREATE TABLE microblog_hashtag( id INT NOT NULL PRIMARY KEY DEFAULT nextval( 'microblog_hashtag_id' ), hashtag VARCHAR( 150 ) UNIQUE NOT NULL );
CREATE TABLE microblog_author( id INT NOT NULL PRIMARY KEY DEFAULT nextval( 'microblog_author_id' ), author VARCHAR( 150 ) UNIQUE NOT NULL, email VARCHAR( 254 ) UNIQUE NOT NULL, auth_token VARCHAR( 64 ) NOT NULL );
